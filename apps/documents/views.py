...
def review(request, pk):
    ...
    # Enforce transport-type consistency before building the form
    ...
    if not b11.get('ktw_medizinisch', False):
        ...
        b12['tragestuhl'] = False
        b12['liegend'] = False
    parsed_data['block11_transport_type'] = b11
    parsed_data['block12_transport_mode'] = b12
    transport_consistency_applied = True
    if transport_consistency_applied:
        upload_obj.parsed_data = parsed_data
        upload_obj.save(update_fields=['parsed_data'])
        logger.info('Transport consistency applied | block11=%s block12=%s', b11, b12)
