...
class DispoliveReportForm(forms.ModelForm):
    ...
    def from_parsed_data(cls, parsed_data):
        ...
        if (initial['patient_country'] or '').strip().lower() == (initial.get('patient_city') or '').strip().lower():
            initial['patient_country'] = 'D'
        if len((initial['patient_country'] or '').strip()) > 3:
            initial['patient_country'] = 'D'
        ...
        if not initial['ktw_medizinisch']:
            initial['rollstuhl'] = False
            initial['tragestuhl'] = False
            initial['liegend'] = False
    ...
    def to_parsed_data(self):
        ...
        if data.get('taxi_mietwagen', False):
            ktw_medizinisch = False
        rollstuhl = data.get('rollstuhl', False)
        tragestuhl = data.get('tragestuhl', False)
        liegend = data.get('liegend', False)
        if not ktw_medizinisch:
            rollstuhl = False
            tragestuhl = False
            liegend = False
        patient_country = data.get('patient_country', 'D')
        if (patient_country or '').strip().lower() == (data.get('patient_city', '') or '').strip().lower():
            patient_country = 'D'
        if len((patient_country or '').strip()) > 3:
            patient_country = 'D'
        return {
            'block1_insurance': {
            ...
            'block2_patient': {
                ...
                'patiant_country': patient_country,
